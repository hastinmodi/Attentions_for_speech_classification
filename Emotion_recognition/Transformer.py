...
def transformer(time_steps, num_layers, units, d_model, num_heads, dropout, output_size, projection, name="transformer"):
  ...
  # We predict the probabilities for our classes using softmax activation
  outputs = tf.keras.layers.Dense(units=output_size,use_bias=True,activation='softmax', name="outputs")(outputs)

  return tf.keras.Model(inputs=[inputs], outputs=outputs, name='audio_class')


OUTPUT_SIZE = 5 # Output size of our model (equal to the number of classes)
D_MODEL = X_train.shape[2] # No. of mfccs
TIME_STEPS = X_train.shape[1] # No. of samples
