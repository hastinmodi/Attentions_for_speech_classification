# Code from the next sources is used
...
df = pd.read_csv('./data_emo_rec.csv')
...
class conf:
    sampling_rate = 16000
    duration = 5 # Results are given for 1, 3 and 5 sec durations
...
def convert_wav_to_mfcc(df, source, folders):
  ...
  for i in df.index:
    if(df.at[i, 'folder'] == folders[0]):
      ...
      x = read_file(conf, source[1] + '/' + str(df.at[i, 'files']), trim_long_data=True)
      X_valid.append(x.transpose())
      Y_valid.append(df.at[i, 'classes'])
    elif(df.at[i, 'folder'] == folders[2]):
      x = read_file(conf, source[2] + '/' + str(df.at[i, 'files']), trim_long_data=True)
      X_test.append(x.transpose())
      Y_test.append(df.at[i, 'classes'])
  return X_train, X_valid, X_test, Y_train, Y_valid, Y_test

train_path = './emotion_recognition/train'
valid_path = './emotion_recognition/valid'
test_path = './emotion_recognition/test'
...
Y_valid = tf.keras.utils.to_categorical(Y_valid, num_classes=n_classes)
...
class ResNest:
    def __init__(self, verbose=True, input_shape=(157, 36, 1), active="relu", n_classes=5,
    ...
    def build(self):
        ...
        fc_out = Dense(self.n_classes, kernel_initializer="he_normal", use_bias=False, activation=self.fc_activation, name="fc_NObias")(x) # replace concats to x
        if self.verbose:
...
def get_model(model_name='ResNest50', input_shape=(157, 36, 1), n_classes=5,
...
log_dir = "./emotion_recognition/logs_5_sec/" + datetime.datetime.now().strftime("%d%m%Y-%H%M%S")
...
checkpoint_path = './emotion_recognition/saved_models_5_sec/' + datetime.datetime.now().strftime("%d%m%Y-%H%M%S")
...
# Load the weights from the checkpoint path for the best model and find the accuracy for the test data
model.load_weights(checkpoint_path)
loss, acc = model.evaluate(X_test, Y_test, batch_size=32, verbose=1)

yhat_probs = model.predict(X_test, verbose=0) # Class probabilities of the test data
yhat_classes = yhat_probs.argmax(axis=1) # Classes predicted by the model of the test data

Y_test = Y_test.argmax(axis=1)

# Finding the model's performance on various metrics
print('ROC AUC: {:.4f}'.format(roc_auc_score(Y_test, yhat_probs, average='macro', multi_class='ovo')))

print('Precision: {:.4f}'.format(precision_score(Y_test, yhat_classes, average='macro')))

print('Recall: {:.4f}'.format(recall_score(Y_test, yhat_classes, average='macro')))

print('F1 score: {:.4f}'.format(f1_score(Y_test, yhat_classes, average='macro')))